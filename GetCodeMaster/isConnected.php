...
$idcon = new mysqli($host, $user, $mdp, $bdd);
	if ( $idcon ->connect_errno ) {
		...
		$sql = "SELECT motdepasse FROM profils WHERE pseudo = '$pseudo'";
		$result = $idcon->query($sql);
		$loged = false;
		if ($ligne = $result->fetch_object() ) {
			$storedpass = $ligne->motdepasse;
			if ($storedpass == $motdepasse) {
				session_start();
				setcookie('nom', $pseudo, time()+3600*24);
				?> <h1 style="text-align:center">Vous êtes connecté.</h1><?php
			}
			else {
				header("Location: http://td1/Connexion.php?$loged");
				exit();
